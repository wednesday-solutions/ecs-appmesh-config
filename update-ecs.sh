if [ ! -f "update.app.properties.json" ] || [ ! -f "app.properties.json" ]; then
...
echo files got $cd_properties_file
...
if [ ! $APP_NAME ] || [ ! $ENV_NAME ] || [ ! $APPMESH_SERVICE_NODE1_TRAFFIC_WEIGHT ]; then
